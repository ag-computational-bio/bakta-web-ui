...
describe('collector', () => {
  function expectAdd(c: Collector, n: number, total: number, currentCount: number, size?: number) {
    c.add(n)
    expect(c.total).toBe(total)
    expect(c.currentCount).toBe(currentCount)
    if (size != undefined) expect(c.currentSize).toBe(size)
  }
  function expectDrop(c: Collector, total: number, currentCount: number, size?: number) {
    c.dropStart()
    expect(c.total).toBe(total)
    expect(c.currentCount).toBe(currentCount)
    if (size != undefined) expect(c.currentSize).toBe(size)
